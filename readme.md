...
const { resumed, reason, action, message } = await client.resumeJob({ jobId });
```
...
would carry on by itself, and a `message` written for the user. A job in any other status is a 409.
