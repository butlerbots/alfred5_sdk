...
export function createLinkHarness(options: { linkId?: string; reconnect?: boolean } = {}): LinkHarness {
    ...
    async function connect(): Promise<FakeSocket> {
        ...
        await flush();
        socket.push("welcome", {
            connectionId: `conn-${sockets.length}`,
            linkId: link.linkId,
            scope: "user",
            protocolVersion: 1,
    ...
    return { link, sockets, connect, answerRegistrations };
}
